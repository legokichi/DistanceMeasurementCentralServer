...
(function() {
  var DSSS_SPEC, MULTIPASS_DISTANCE, SOUND_OF_SPEED, TEST_INPUT_MYSELF, VIEW_SIZE, _craetePictureFrame, _lowpass, _prepareRec, _prepareSpect, actx, analyser, beepPulse, changeColor, collect, isBroadcasting, isRecording, main, osc, processor, pulseStartTime, pulseStopTime, ready, recbuf, sendRec, socket, startPulse, startRec, stopPulse, stopRec;
  ...
  collect = function(datas) {
    return function(next) {
      var aliases, delayTimes, distances, distancesAliased, frame_, pulseTimes, relDelayTimes, results, sampleRates;
      ...
      results = datas.map(function(arg) {
        var DSSS_SPEC, PULSE_N, _results, alias, frame, id, recF32arr, sampleRate, startStops;
        ...
        PULSE_N = DSSS_SPEC.PULSE_N;
        frame = _craetePictureFrame(alias + "@" + id);
        frame_.add(frame.element);
        _results = startStops.map(function(arg1) {
          var T, _frame, _id, _target, a, b, correl, correlL, correlR, correl_full, correls, i, idL, idR, length, marker, matched, max_offset, maxes, normalize_val, offset, pulseTime, raked, ref, ref1, section, src, srcL, srcR, src_self, srcview, startPtr, stopPtr, summed, target, target_self, text, trgview, valL, valR, view;
          ...
          correls = (function() {
            var j, ref, results1;
            results1 = [];
            for (i = j = 0, ref = correl.length / T | 0; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
              marker[T * i] = 255;
              marker[T * i + T] = 255;
              a = correl.subarray(T * i, T * i + T);
              if (a.length === T) {
                results1.push(a);
              } else {
          ...
          summed.forEach(function(_, i) {
            return summed[i] = correls.reduce((function(a, v) {
              return a + v[i];
            }), 0);
          });
          view(summed, "summed");
          maxes = correls.map(function(correl, i) {
            return {
              val: Signal.fft_smart_overwrap_correlation(correl, summed)[0],
              i: i
            };
          });
          text("maxes:" + (JSON.stringify(maxes)));
          _target = maxes.sort(function(a, b) {
            return a.val < b.val;
          })[0];
          ...
          raked = Signal.fft_smart_overwrap_correlation(target, src).map(function(v, i) {
            return v / normalize_val;
          });
          view(raked, "raked(0)=" + raked[0]);
          offset = 0;
          length = src.length;
          while (length > Math.pow(2, 10)) {
            ...
            correlL = Signal.fft_smart_overwrap_correlation(trgview, srcL).map(function(v, i) {
              return v / normalize_val;
            });
            correlR = Signal.fft_smart_overwrap_correlation(trgview, srcR).map(function(v, i) {
              return v / normalize_val;
            });
            view(correlL, "correlL");
            view(correlR, "correlR");
            text((ref = Signal.Statictics.findMax(correlL), valL = ref[0], idL = ref[1], ref));
            text((ref1 = Signal.Statictics.findMax(correlR), valR = ref1[0], idR = ref1[1], ref1));
      ...
      (function() {
        var K, basePt, ds, pseudoPts, render, sdm;
