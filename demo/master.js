...
(function() {
  var HEIGHT, MULTIPASS_DISTANCE, PADDING, SOUND_OF_SPEED, TIME_DATA, VIEW_SIZE, VS, WIDTH, _craetePictureFrame, _processing, calc, main, points, socket;
  ...
  _processing = function(next) {
    ...
    new Processing($("#_cnv_")[0], function(_) {
      ...
      return _.draw = function() {
        var R, __l, __x, __y, _x, _y, a, ds, i, j, k, l, len, len1, m, n, name, point, q, r, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, vs, x, y;
        if (drgTrgPtr != null) {
        ...
        if (drgTrgPtr === VS) {
          R = 6;
          a = R / (20 * Math.log10(2));
          ds = points.map(function(point) {
            var _x, _y, x, y;
            x = point[0], y = point[1];
            _x = VS[0], _y = VS[1];
            return Math.sqrt(Math.pow(x - _x, 2) + Math.pow(y - _y, 2));
          });
          k = 1 / Math.sqrt(ds.reduce((function(a, d) {
            return a + 1 / Math.pow(d, 2 * a);
          }), 0));
          vs = ds.map(function(d) {
            return k / Math.pow(d, a);
          });
          socket.emit("set_vs_weight", vs.reduce((function(o, v, i) {
            var alias, id, ref, x, y;
            ref = points[i], x = ref[0], y = ref[1], alias = ref[2], id = ref[3];
            o[id] = v;
            return o;
          }), {}));
        }
        _.background(255);
        _.stroke(127);
        for (i = l = ref = PADDING, ref1 = _.width, ref2 = PADDING; ref2 > 0 ? l <= ref1 : l >= ref1; i = l += ref2) {
          _.line(0, i, _.width, i);
        }
        for (i = m = ref3 = PADDING, ref4 = _.height, ref5 = PADDING; ref5 > 0 ? m <= ref4 : m >= ref4; i = m += ref5) {
        ...
        for (i = r = 0, len1 = points.length; r < len1; i = ++r) {
          point = points[i];
          x = point[0], y = point[1], name = point[2];
          _.fill(255);
          _.ellipse(x, y, 25, 25);
          _.fill(0);
          _.text(name, x - 3, y + 5);
