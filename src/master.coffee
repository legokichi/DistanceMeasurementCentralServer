...
calc = (datas)-> (next)->
  ...
  results = datas.map ({id, alias, startStops, recF32arr, DSSS_SPEC, sampleRate})->
    ...
    _results = startStops.map ({id: _id, startPtr, stopPtr})->
      ...
      threshold = 80
      stdscore_section_matched_range = low_section_matched_range.map (x)-> 10 * (x - ave) / vari + PADDING
      flag = true
      while flag
        for v, offset in stdscore_section_matched_range
          if threshold < stdscore_section_matched_range[offset]
            #if med < low_section_matched_range[offset]
            #  if ave < low_section_matched_range[offset]
                flag = false
                break
        threshold -= 1
  ...
  new Processing $("#_cnv_")[0], (_)->
    ...
    _.draw = ->
      ...
      if drgTrgPtr is VS
        ...
        k = 1/ Math.sqrt(ds.reduce(((a, d)-> a+1/Math.pow(d, 2 * a)), 0))
        vs = ds.map (d)-> k / Math.pow(d, a)
        socket.emit("set_vs_weight", vs.reduce(((o, v, i)->
          [x, y, alias, id] = points[i]
          o[id] = v
          o), {}))
      _.background(255)
      # 罫線
      _.stroke(127)
      _.line(0, i, _.width, i)  for i in [PADDING.._.width] by PADDING
      _.line(i, 0, i, _.height) for i in [PADDING.._.height] by PADDING
      _.stroke(0)
      # 相対線
      for point, i in points
        ...
        for j in [i...points.length] when i != j
          ...
          __l = Math.sqrt(
                          Math.pow(x - _x, 2) +
                          Math.pow(y - _y, 2))
          _.text((__l/PADDING*100 | 0)/100, __x, __y)
      # 丸
      for point, i in points
        [x, y, name] = point
        _.fill(255)
