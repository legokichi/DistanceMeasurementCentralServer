...
(function() {
  ...
  this.SignalViewer = (function() {
    ...
    SignalViewer.prototype.draw = function(f32arr, opt) {
      var _, arr, detail, i, k, len, max, min, ref, ref1, sampleRate, v;
      if (opt == null) {
        opt = {};
      }
      sampleRate = opt.sampleRate;
      arr = new Float32Array(f32arr.length);
      for (i = k = 0, len = f32arr.length; k < len; i = ++k) {
        v = f32arr[i];
        arr[i] = isFinite(v) ? v : 0;
      }
      if (sampleRate == null) {
        sampleRate = 44100;
      }
    ...
    SignalViewer.prototype.drawSpectrogram = function(f32arr, opt) {
      var _, _f32arr, arr, b, buffer, detail, g, i, imgdata, index, j, k, l, len, len1, len2, len3, m, max, n, ptr, r, ref, ref1, sampleRate, slideWidth, spectrum, spectrums, v, windowSize, x, y;
      if (opt == null) {
        opt = {};
      }
      sampleRate = opt.sampleRate, windowSize = opt.windowSize, slideWidth = opt.slideWidth, max = opt.max;
      _f32arr = new Float32Array(f32arr.length);
      for (i = k = 0, len = f32arr.length; k < len; i = ++k) {
        v = f32arr[i];
        _f32arr[i] = isFinite(v) ? v : 0;
      }
      f32arr = _f32arr;
      ...
      while (ptr + windowSize < arr.length) {
        ...
        for (i = l = 0, len1 = spectrum.length; l < len1; i = ++l) {
          _ = spectrum[i];
      ...
      for (i = m = 0, len2 = spectrums.length; m < len2; i = ++m) {
        _ = spectrums[i];
        for (j = n = 0, len3 = spectrum.length; n < len3; j = ++n) {
          _ = spectrum[j];
