...
(function() {
  var MULTIPASS_DISTANCE, OSC, SOUND_OF_SPEED, Signal, SignalViewer, VIEW_SIZE, craetePictureFrame;
  ...
  this.Detector = (function() {
    ...
    Detector.prototype.init_barkerCodedChirp = function(_, next) {
      var bcc;
    ...
    Detector.prototype.calc_barker = function(rawdata, sampleRate) {
      return (function(_this) {
        return function(arg) {
          var correlA, frame, id, idxA, marker, maxA, max_offset, max_val, pulseTime, range, ref, startPtr, stopPtr, zoomA;
    ...
    Detector.prototype.calc_mseq = function(rawdata, sampleRate) {
      return (function(_this) {
        return function(arg) {
          var _, _idx, _logs, correl, correlA, correlB, frame, i, id, idx, idxA, idxB, j, len, logs, marker, max, maxA, maxB, max_offset, max_val, pulseTime, range, ref, ref1, ref2, ref3, ref4, relA, relB, scoreA, scoreB, slidewidth, startPtr, stdscoreA, stdscoreB, stopPtr, val, windowsize, zoom, zoomA, zoomB;
          id = arg.id, startPtr = arg.startPtr, stopPtr = arg.stopPtr;
          frame = craetePictureFrame(socket.id + "<->" + id, document.body);
          correlA = Signal.fft_smart_overwrap_correlation(rawdata, _this.matchedA);
          ...
          zoom = new Float32Array(zoomA.length);
          for (i = j = 0, len = zoomA.length; j < len; i = ++j) {
            _ = zoomA[i];
            zoom[i] = zoomA[i] * zoomB[i];
          }
